...
beta_star <- c(experiment_setting$beta1, experiment_setting$beta2, rep(0, 98))
...
df_lst <- future_lapply(reps, \(ii) {
  ...
  df_est <- data.frame(matrix(0, nrow = 0, ncol = length(beta_star) + 2))
  colnames(df_est) <- c(paste0("beta", 1:length(beta_star), "_hat"), "method", "n")
  ...
  for (kk in 1:length(n_grid)) {
    ...
    dat_sumstat <- dgp2(ny = n_grid[kk], nx = n_grid[kk], 
                        beta_star = beta_star, type = "depen_mv")
    
    # TSIV-Lasso with summary statistics
    beta_hat <- grid_search_iv_lasso_sumstat(beta_init = rep(0.5, length(beta_star)), 
    ...
    rmse[1] <- sqrt(sum((beta_star - beta_hat)^2))
    bias[1] <- sum(abs(beta_star - beta_hat))
    ...
    rmse[2] <- sqrt(sum((beta_star - beta_hat)^2))
    bias[2] <- sum(abs(beta_star - beta_hat))
    ...
    df_res <- rbind(
      ...
      data.frame(rmse = rmse,
                 bias = bias,
                 method = c("IV-Lasso-sumstat-refit", 
                            "IV-sumstat"),
...
saveRDS(experiment_result, 
        file = paste0("result/dgp2_2sample_sumstat.rds"))
