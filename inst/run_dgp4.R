...
df_lst <- future_lapply(reps, \(ii) {
  devtools::load_all()
  dat1 <- dgp_noniden1(sum(n_grid), beta1, beta2)
  dat2 <- dgp_noniden1(sum(n_grid), beta1, beta2)
  ...
  for (kk in 1:length(n_grid)) {
    ...
    dat_sumstat <- comp_sumstat(dat1$Y[n_index[[kk]],,drop=FALSE], dat1$Z[n_index[[kk]],,drop=FALSE], 
                                dat2$X[n_index[[kk]],,drop=FALSE], dat2$Z[n_index[[kk]],,drop=FALSE], 
                                type = "depen_mv")
    ...
    # TSIV-Lasso with summary statistics (refit)
...
saveRDS(experiment_result, 
        file = paste0("result/new_dgp4_2sample_sumstat.rds"))
