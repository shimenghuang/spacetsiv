...
## summarize results of run_dgp2.R

# load saved results
experiment_result <- readRDS("script/result/dgp2_2sample_sumstat.rds")
list2env(experiment_result, .GlobalEnv)
beta_star <- c(beta1, beta2, rep(0, 98))
alpha <- 0.05
...
df_res$method <- factor(df_res$method, levels = c("IV-Lasso-sumstat-refit", 
                                                  "IV-sumstat"))
...
p1 <- ggplot(df_res_long, 
  ...
  scale_color_manual(values = c("#74C476", "#FDAE61"),
                     labels = c("IV-Lasso-sumstat-refit" = "spaceTSIV-L1",
                                "IV-sumstat" = "TSIV"),
                     name = "estimator") +
  theme_bw() +
...
df_est <- do.call(rbind, df_lst_est) %>%
  tidyr::pivot_longer(1:100, names_to = "beta_hat", values_to = "est") %>%
  dplyr::mutate(n = factor(n),
                method = factor(method, levels = c("IV-Lasso-sumstat-refit", 
                                                   "IV-sumstat"))) %>%
  dplyr::group_by(method, n) %>%
  dplyr::mutate(star = rep(beta_star, length(reps)),
                iter = rep(reps, each = length(beta_star))) %>%
  dplyr::mutate(beta_hat = factor(beta_hat))

p3 <- ggplot(df_est %>%
               dplyr::filter(beta_hat %in% c("beta1_hat", "beta2_hat", "beta3_hat", "beta4_hat")), 
             aes(x = n, y = est, color = beta_hat)) + 
  geom_boxplot(width = 0.5, position = position_dodge(0.75), outlier.alpha = 0) + 
  ggbeeswarm::geom_quasirandom(dodge.width = 0.75, size = 0.5, alpha = 0.75) +
  geom_hline(linetype = 2, aes(yintercept = star, color = beta_hat)) + 
  facet_grid(method~., labeller = labeller(method = c("IV-Lasso-sumstat-refit" = "spaceTSIV-L1",
                                                      "IV-sumstat" = "TSIV"))) +
  xlab("sample size") + 
  ylab("Estimates") +
  scale_color_manual(values = colorspace::qualitative_hcl(4,
                                                          alpha = 0.75)) +
...
ggsave(paste0("script/result/dgp2_2sample.pdf"), 
       plot = p1, width = 5, height = 3.5)

ggsave(paste0("script/result/dgp2_2sample_estimates.pdf"), 
...
data.table::fwrite(x = df_out, 
                   paste0("script/result/dgp2_2sample.csv"))
...
select_res <- rbind(res_jaccard, res_sparsity) %>%
  dplyr::filter(method %in% c("Q-subset-sumstat", "IV-Lasso-sumstat-refit")) %>%
  dplyr::mutate(name = factor(name, 
                              levels = c("jaccard", "correct_sparsity_pct")))
...
p4 <- select_res %>%
  ...
  ggplot2::facet_grid(name ~., scale = "free_y", 
                      labeller = as_labeller(c("jaccard" = "Jaccard", 
                                               "correct_sparsity_pct" = "correct size (%)"))) + 
  ggplot2::labs(y = element_blank()) + 
  ggplot2::scale_color_manual(values = c("#74C476"),
                              labels = c("IV-Lasso-sumstat-refit" = "spaceTSIV-L1"),
                              name = "estimator") + 
  ggplot2::theme_bw() +
  ggplot2::theme(legend.position = "top",
...
p <- ggpubr::ggarrange(p1, p4, nrow = 1, common.legend = TRUE,
                       align = "h")

ggsave(paste0("script/result/dgp2_2sample_combined.pdf"), 
       plot = p, width = 10.2, height = 5.2)
