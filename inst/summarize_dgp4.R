...
# load saved results
experiment_result <- readRDS("inst/result/dgp4_2sample_sumstat.rds")
list2env(experiment_result, .GlobalEnv)
beta_star <- c(beta1, beta2, 0, 0)
alpha <- 0.05
...
p1 <- ggplot(df_res_long %>%
  ...
  scale_color_manual(values = c("#5E4FA2", 
                                # "#4292C6", 
                                "#74C476", "#FDAE61"),
...
df_est <- do.call(rbind, df_lst_est) %>%
  dplyr::filter(method %in% c("Q-subset-sumstat",
                              "IV-Lasso-sumstat-refit",
                              "IV-sumstat")) %>%
  tidyr::pivot_longer(1:4, names_to = "beta_hat", values_to = "est") %>%
...
df_est_set <- df_est %>%
  ...
  dplyr::summarise(est_set = paste(which(est != 0), collapse = "_")) %>%
  dplyr::mutate(est_set = factor(est_set, levels = c('1', '2', '3', '4', 
                                                     '1_2', '1_4', '2_3', '3_4', '1_3', '2_4', 
                                                     '1_2_3', '1_2_4', '2_3_4',
                                                     '1_2_3_4', '')))

p2 <- df_est_set %>%
  ggplot(aes(x = est_set, fill = method)) +
  ...
  ggplot2::theme(legend.position = "top",
                 ...
                 plot.margin = margin(3, 0, 0, 0),
                 text = element_text(size = 13)) + 
  scale_fill_manual(values = c("#5E4FA2", "#74C476", "#FDAE61"),
                     labels = c("Q-subset-sumstat" = "spaceTSIV-L0",
                                "IV-Lasso-sumstat-refit" = "spaceTSIV-L1",
                                "IV-sumstat" = "TSIV"),
                     name = "estimator") + 
  labs(x = "estimated causal set") + 
  ggplot2::guides(color = guide_legend(nrow = 1, byrow = FALSE))

ggsave(paste0("inst/result/dgp4_2sample_sets.pdf"), 
       plot = p2, width = 5, height = 3.5)
...
p3 <- ggplot(df_est, 
  ...
  facet_grid(method~., labeller = labeller(method = c("Q-subset-sumstat" = "spaceTSIV-L0",
                                                      "IV-Lasso-sumstat-refit" = "spaceTSIV-L1 (refit)",
                                                      "IV-Lasso-sumstat-norefit" = "spaceTSIV-L1 (no refit)",
                                                      "IV-sumstat" = "TSIV"))) +
  xlab("sample size") + 
  ylab("Estimates") +
...
ggsave(paste0("inst/result/dgp4_2sample.pdf"), 
       plot = p1, width = 5, height = 3.5)

ggsave(paste0("inst/result/dgp4_2sample_estimates.pdf"), 
...
data.table::fwrite(x = df_out, 
                   paste0("inst/result/dgp4_2sample.csv"))

# confusion matrix
confusion_matrix <- df_est %>%
  # selections are the same for refit and no refit 
...
ggsave(paste0("inst/result/dgp4_2sample_combined.pdf"), 
